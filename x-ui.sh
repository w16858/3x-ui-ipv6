...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/w16858/x-ui-ipv6/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
custom_version() {
    ...
    download_link="https://raw.githubusercontent.com/w16858/x-ui-ipv6/master/install.sh"
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.via-ipv6.com/w16858/x-ui-ipv6/raw/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
update_geo() {
    ...
    wget -N https://github.via-ipv6.com/Loyalsoldier/v2ray-rules-dat/releases/latest/download/geoip.dat
    wget -N https://github.via-ipv6.com/Loyalsoldier/v2ray-rules-dat/releases/latest/download/geosite.dat
    wget -O geoip_IR.dat -N https://github.via-ipv6.com/chocolate4u/Iran-v2ray-rules/releases/latest/download/geoip.dat
    wget -O geosite_IR.dat -N https://github.via-ipv6.com/chocolate4u/Iran-v2ray-rules/releases/latest/download/geosite.dat
    wget -O geoip_VN.dat https://github.via-ipv6.com/vuong2023/vn-v2ray-rules/releases/latest/download/geoip.dat
    wget -O geosite_VN.dat https://github.via-ipv6.com/vuong2023/vn-v2ray-rules/releases/latest/download/geosite.dat
    systemctl start x-ui
...
ssl_cert_issue() {
    ...
    *)
    ...
    ~/.acme.sh/acme.sh --issue -d ${domain} --standalone --httpport ${WebPort} --listen-v6
    if [ $? -ne 0 ]; then
